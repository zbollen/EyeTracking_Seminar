...
criterion_name <- "Gaze points during the period of interest"
...
criterion_name <- "Valid gaze points"
...
criterion_name <- "Gaze points inside the screen"
...
## Exclude trials with half of fixations outside AOIs

```{r}
# Criterion 4: Trials with min half of fixations inside AOIs
criterion_name <- "Trials with > 50% fixations inside AOIs"
...
# After filtering
removed <- ifelse(nrow(trials_to_exclude) > 0, nrow(trials_to_exclude), 0)
kept <- total_trials - removed
percentage_removed <- (removed / total_rows) * 100
...
```{r}
# Criterion 5: Fixations inside AOIs
criterion_name <- "Fixations inside AOIs"

# Number of trials before filtering
total_fixations <- nrow(All_Data_cleaned)
